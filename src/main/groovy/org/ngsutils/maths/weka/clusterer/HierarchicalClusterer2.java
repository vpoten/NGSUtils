 ...
 * @author victor
 */
public class HierarchicalClusterer2 {//extends AbstractClusterer implements OptionHandler, CapabilitiesHandler, Drawable {
...
//            }
//
//            public void setValues(double d, int i, int j, int nSize1, int nSize2) {
//                    m_fDist = d;
//                    m_iCluster1 = i;
//                    m_iCluster2 = j;
//                    m_nClusterSize1 = nSize1;
...
//                DecimalFormat myFormatter = new DecimalFormat("#.#####",DecimalFormatSymbols.getInstance(Locale.ENGLISH));
//
//                if (m_left == null) {
//                    if (m_right == null) {
//                            return "(" + getInstances().instance(m_iLeftInstance).stringValue(attIndex) + ":" + myFormatter.format(m_fLeftLength) + "," +
//                                         getInstances().instance(m_iRightInstance).stringValue(attIndex) +":" + myFormatter.format(m_fRightLength) + ")";
//                    } else {
//                            return "(" + getInstances().instance(m_iLeftInstance).stringValue(attIndex) + ":" + myFormatter.format(m_fLeftLength) + "," +
...
//                if (m_left == null) {
//                    if (m_right == null) {
//                            return "(" + getInstances().instance(m_iLeftInstance).value(attIndex) + ":" + myFormatter.format(m_fLeftLength) + "," +
//                                         getInstances().instance(m_iRightInstance).value(attIndex) +":" + myFormatter.format(m_fRightLength) + ")";
//                    } else {
//                            return "(" + getInstances().instance(m_iLeftInstance).value(attIndex) + ":" + myFormatter.format(m_fLeftLength) + "," +
...
//		m_instances = data;
//		int nInstances = getInstances().numInstances();
//		if (nInstances == 0) {
//			return;
//		}
//		m_DistanceFunction.setInstances(getInstances());
//		// use array of integer ArrayLists to store cluster indices,
//		// starting with one cluster per instance
//		ArrayList<Integer> [] nClusterID = new ArrayList[data.numInstances()];
...
//		} else {
//                    //this method don't uses priority queue
//                    doLinkClusteringLowMemory(nClusters, nClusterID, clusterNodes);
//
...
//	 * @param clusterNodes
//	 */
//	void neighborJoining(int nClusters, ArrayList<Integer>[] nClusterID, Node [] clusterNodes) {
//            int n = getInstances().numInstances();
//
...
//				for(int i = 0; i < n; i++) {
//				    if(i == iMin1 || i == iMin2 || nClusterID[i].isEmpty()) {
//				        fRow1[i] = 0;
//				    } else {
...
//				while (nClusterID[iPrev].isEmpty()) {
//					iPrev--;
//				}
...
//	 * @param clusterNodes
//	 */
//	void doLinkClustering(int nClusters, ArrayList<Integer>[] nClusterID, Node [] clusterNodes) {
//            int nInstances = getInstances().numInstances();
//
//            PriorityQueue<Tuple> queue = new PriorityQueue<Tuple>(nClusters*nClusters/2, new TupleComparator());
//            TriangularMatrix fDistance0 = new TriangularMatrix(nClusters);
//            TriangularMatrix fClusterDistance = null;
//
//            if (m_bDebug) {
//                fClusterDistance = new TriangularMatrix(nClusters);
//            }
//            for (int i = 0; i < nClusters; i++) {
//                fDistance0.setValue(i, i, 0);
//                for (int j = i+1; j < nClusters; j++) {
//                    fDistance0.setValue(i, j, getDistance0(nClusterID[i], nClusterID[j]) );
//                    queue.add(new Tuple( fDistance0.getValue(i, j), i, j, 1, 1));
//                    if (m_bDebug) {
//                        fClusterDistance.setValue(i, j, fDistance0.getValue(i, j) );
//                    }
//                }
//            }
...
//                                if (nClusterID[j].size()>0) {
//                                    double fDist = fClusterDistance.getValue(i, j);
...
//                        if (m_bDebug) {
//                            if( i1<i2 )
//                                fClusterDistance.setValue(i1, i2, fDistance );
//                            else
//                                fClusterDistance.setValue(i2, i1, fDistance );
//                        }
//                        
...
//	 * @param clusterNodes
//	 */
//	void doLinkClusteringLowMemory(int nClusters, ArrayList<Integer>[] nClusterID, Node [] clusterNodes) {
//            int nInstances = getInstances().numInstances();
//
//            TriangularMatrix fDistance0 = new TriangularMatrix(nClusters);
//            TriangularMatrix fClusterDistance = new TriangularMatrix(nClusters);
//
//            for (int i = 0; i < nClusters; i++) {
//                fDistance0.setValue(i, i, 0);
//                for (int j = i+1; j < nClusters; j++) {
...
//	} // doLinkClustering2
//
//	void merge(int iMin1, int iMin2, double fDist1, double fDist2, ArrayList<Integer>[] nClusterID, Node [] clusterNodes) {
//		if (m_bDebug) {
...
//	double getDistance0(ArrayList<Integer> cluster1, ArrayList<Integer> cluster2) {
...
//			Instance instance1 = getInstances().instance(cluster1.get(0));
//			Instance instance2 = getInstances().instance(cluster2.get(0));
...
//	 */
//	
//        double getDistance( TriangularMatrix fDistance, ArrayList<Integer> cluster1, ArrayList<Integer> cluster2) {
//		double fBestDist = Double.MAX_VALUE;
//		switch (m_nLinkType) {
//		case SINGLE:
...
//					double fDist = fDistance.getValue2(i1,i2);
//					if (fBestDist > fDist) {
//						fBestDist = fDist;
//					}
//				}
...
//				for (int j = 0; j < cluster2.size(); j++) {
//					int i2 = cluster2.get(j);
//					fBestDist += fDistance.getValue2(i1,i2);
//				}
//			}
...
//			double [] fValues1 = new double[getInstances().numAttributes()];
//			for (int i = 0; i < cluster1.size(); i++) {
//				Instance instance = getInstances().instance(cluster1.get(i));
//				for (int j = 0; j < getInstances().numAttributes(); j++) {
//					fValues1[j] += instance.value(j);
//				}
//			}
//			double [] fValues2 = new double[getInstances().numAttributes()];
//			for (int i = 0; i < cluster2.size(); i++) {
//				Instance instance = getInstances().instance(cluster2.get(i));
//				for (int j = 0; j < getInstances().numAttributes(); j++) {
...
//			}
//			// set up two instances for distance function
//			Instance instance1 = (Instance) getInstances().instance(0).copy();
//			Instance instance2 = (Instance) getInstances().instance(0).copy();
//			for (int j = 0; j < getInstances().numAttributes(); j++) {
...
//			Instance instance = getInstances().instance(cluster.get(i));
//			for (int j = 0; j < getInstances().numAttributes(); j++) {
//				fValues1[j] += instance.value(j);
//			}
//		}
...
//		}
//		// set up two instances for distance function
//		Instance centroid = (Instance) getInstances().instance(cluster.get(0)).copy();
//		for (int j = 0; j < getInstances().numAttributes(); j++) {
...
//		if (getInstances().numInstances() == 0) {
//			return 0;
//		}
//		double fBestDist = Double.MAX_VALUE;
//		int iBestInstance = -1;
//		for (int i = 0; i < getInstances().numInstances(); i++) {
//			double fDist = m_DistanceFunction.distance(instance, getInstances().instance(i));
//			if (fDist < fBestDist) {
//				fBestDist = fDist;
//				iBestInstance = i;
//			}
...
//		return Math.min(m_nNumClusters, getInstances().numInstances());
//	}
//
//	  /**
...
//
//	    ArrayList newArrayList = new ArrayList(8);
//	    newArrayList.add(new Option(
...
//              int attIndex = getInstances().classIndex();
//              if (attIndex < 0) {
//                      // try find a string, or last attribute otherwise
//                      attIndex = 0;
//                      while (attIndex < getInstances().numAttributes()-1) {
//                          if (getInstances().attribute(attIndex).isString()) {
//                                  break;
//                          }
//                          attIndex++;
//                      }
...
//                                          buf.append("Cluster ").append(i).append("\n");
//                                          if (getInstances().attribute(attIndex).isString()) {
...
//                      }
//            } catch (Exception ex) {
//                Logger.getLogger(HierarchicalClusterer2.class.getName()).log(Level.SEVERE, null, ex);
//            }
//              return buf.toString();
//      }
//
...
//    /**
//     *
//     * @param cutValue : node height cut value
//     * @return
//     */
//    public Set<Set<Instance>> getClustersGroups( double cutValue ){
//
//        Set<Set<Instance>> groups=new HashSet<Set<Instance>>();
//
//        ArrayDeque<Node> deque=new ArrayDeque<Node>();
//        deque.add( m_clusters[0] );
//
//        while( !deque.isEmpty() ){
//            Node current=deque.poll();
//
//            if( current.m_fHeight<cutValue ){
//                //get all leaves and add them to a cluster
//                ArrayDeque<Node> deque2=new ArrayDeque<Node>();
//                deque2.add( current );
//
//                Set<Instance> cluster=new HashSet<Instance>();
//
//                
//                while( !deque2.isEmpty() ){
//                    Node subtree=deque2.poll();
//
//                    if( subtree.m_left!=null )
//                        deque2.add(subtree.m_left);
//                    else
//                        cluster.add( getInstances().instance(subtree.m_iLeftInstance) );
//
//                    if( subtree.m_right!=null )
//                        deque2.add(subtree.m_right);
//                    else
//                        cluster.add( getInstances().instance(subtree.m_iRightInstance) );
//                }
//
//                groups.add(cluster);
//            }
...
//                    deque.add(current.m_left);
//                }
//                else{
//                    //add a singleton cluster
//                    Set<Instance> cluster=new HashSet<Instance>();
//                    cluster.add( getInstances().instance(current.m_iLeftInstance) );
//                    groups.add(cluster);
//                }
//
//                if( current.m_right!=null ){
//                    deque.add(current.m_right);
//                }
//                else{
//                    //add a singleton cluster
//                    Set<Instance> cluster=new HashSet<Instance>();
//                    cluster.add( getInstances().instance(current.m_iRightInstance) );
//                    groups.add(cluster);
//                }
//            }
//        }
